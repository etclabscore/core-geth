...
func newFreezerRemote(datadir string, namespace string) (*freezerRemote, error) {
	fmt.Println("---------------------------FREEZAAA RULESSS===========")
...
func (f *freezerRemote) freeze(db ethdb.KeyValueStore) {
	/*
	...
	*/
	...
	for {
		...
		case <-f.quit:
			...
			log.Info("FREEZAAAA shutting down")
			return
		default:
		}
		if backoff {
		...
		/*
			...
			case *number-vars.ImmutabilityThreshold <= f.frozen:
