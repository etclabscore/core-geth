...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig ctypes.ChainConfigurator, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	...
	if frozen, err := bc.db.Ancients(); err == nil && frozen > 0 {
		...
		// blockchain getLatestBlockWithState. If the head full block is even lower than the ancient
...
// getLatestBlockWithState tries to repair the blockchain by iterating backwards from the current block
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		...
		log.Error("Impossible reorg, please file an issue", "oldnum", oldBlock.Number(), "oldhash", oldBlock.Hash(), "newnum", newBlock.Number(), "newhash", newBlock.Hash(), "oldchain.len", len(oldChain), "newchain.len", len(newChain))
		return fmt.Errorf(fmt.Sprint("impossible reorg", "oldnum", oldBlock.Number(), "oldhash", oldBlock.Hash(), "newnum", newBlock.Number(), "newhash", newBlock.Hash(), "oldchain.len", len(oldChain), "newchain.len", len(newChain)))
	}
