...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	tx2, _ := types.SignTx(types.NewTransaction(1, userAddr1, big.NewInt(10000), vars.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	tx3, _ := types.SignTx(types.NewTransaction(2, userAddr1, big.NewInt(10000), vars.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
