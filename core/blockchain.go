...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	reorg := externTd.Cmp(localTd) > 0 // EIP-3436 (1): Choose the block with the most total difficulty.
	currentBlock = bc.CurrentBlock()
	if !reorg && externTd.Cmp(localTd) == 0 {
		// Split same-difficulty blocks by number, then preferentially select
		// the block generated by the local miner as the canonical block.
		if block.NumberU64() < currentBlock.NumberU64() {
			reorg = true // EIP-3436 (2): Choose the block with the lowest block number.

		} else if block.NumberU64() == currentBlock.NumberU64() && bc.Config().GetConsensusEngineType().IsEthash() {
			// This scope only applies to Ethash protocol.
			// Do not run this scope for Clique protocols; it is not specified, and yields nondeterministic positive results.
			// See comments below for details.

			var currentPreserve, blockPreserve bool

			// This method bc.shouldPreserve (see func (s *Ethereum) shouldPreserve(block *types.Block) bool)
			// is, as hardcoded, always returns FALSE for Clique.
			if bc.shouldPreserve != nil {

				// For Clique always: false, false
				currentPreserve, blockPreserve = bc.shouldPreserve(currentBlock), bc.shouldPreserve(block)
			}
			// For Clique always: !false==true && (false || 50%) => 50%
			reorg = !currentPreserve && (blockPreserve || mrand.Float64() < 0.5)

		} else if block.NumberU64() == currentBlock.NumberU64() && bc.Config().GetConsensusEngineType().IsClique() &&

			// Only run rules 3 and 4 if their specifying EIP-3436 is enabled from the current block.
			// This is not strictly necessary since the upgrade is soft and does not need an activation number.
			// However using an activation number is allowable within that constraint (of having no activation constraint),
			// and allows the feature to be configurable.
			bc.Config().IsEnabled(bc.Config().GetCliqueEIP3436Transition, currentBlock.Number()) {
			...
			// Rule 3: here
			// Rule 4: here
			reorg, err = bc.Engine().(*clique.Clique).Eip3436Rule3rule4(bc, currentBlock.Header(), block.Header())
			if err != nil {
				log.Error("Clique EIP-3436 errored", "error", err)
				return NonStatTy, err
			}
		}
	}
