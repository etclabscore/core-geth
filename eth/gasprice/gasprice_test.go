...
func newTestBackend(t *testing.T, londonBlock *big.Int, cancunBlock *big.Int, pending bool) *testBackend {
		...
		if cancunBlock != nil && b.Number().Cmp(cancunBlock) >= 0 {
			...
			for j := 0; j < i && j < 6; j++ {
				blobTx := &types.BlobTx{
					...
					To:         common.Address{},
					Gas:        30000,
					GasFeeCap:  uint256.NewInt(100 * vars.GWei),
					GasTipCap:  uint256.NewInt(uint64(i+1) * vars.GWei),
					Data:       []byte{},
	...
	gspec.Config.SetEthashTerminalTotalDifficulty(new(big.Int).SetUint64(td))
