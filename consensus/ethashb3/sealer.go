		...
		default:
			...
			if powBuffer.SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("EthashB3 nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("EthashB3 nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func startRemoteSealer(ethash *EthashB3, urls []string, noverify bool) *remoteSealer {
...
func (s *remoteSealer) loop() {
	defer func() {
		s.ethash.config.Log.Trace("EthashB3 remote sealer is exiting")
		s.cancelNotify()
		s.reqWG.Wait()
		close(s.exitCh)
	}()
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
	...
	if s.results == nil {
		s.ethash.config.Log.Warn("EthashB3 result channel is empty, submitted mining result is rejected")
		return false
	}
