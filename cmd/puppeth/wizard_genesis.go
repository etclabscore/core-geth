...
func (w *wizard) makeGenesis() {
	...
	genesis := &genesis2.Genesis{
		...
		Alloc:      make(genesis2.GenesisAlloc),
	...
	for {
		...
		if address := w.readAddress(); address != nil {
			genesis.Alloc[*address] = genesis2.GenesisAccount{
	...
	if w.readDefaultYesNo(true) {
		...
		for i := int64(0); i < 256; i++ {
			genesis.Alloc[common.BigToAddress(big.NewInt(i))] = genesis2.GenesisAccount{Balance: big.NewInt(1)}
		}
	}
