...
func (oracle *Oracle) processBlock(bf *blockFees, percentiles []float64) {
	...
	if config.IsEnabled(config.GetEIP1559Transition, big.NewInt(int64(bf.blockNumber+1))) {
	...
	if blobGasUsed := bf.header.BlobGasUsed; blobGasUsed != nil {
		bf.results.blobGasUsedRatio = float64(*blobGasUsed) / vars.MaxBlobGasPerBlock
	}
