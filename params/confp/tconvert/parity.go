...
func NewParityChainSpec(network string, genesis *genesisT.Genesis, bootnodes []string) (*parity.ParityChainSpec, error) {
	...
	if err := confp.Convert(spec, genesis); err != nil {
		return nil, err
	}
	if err := confp.Convert(spec, genesis.Config); err != nil {
		return nil, err
	}
	return spec, nil
}
...
func ParityConfigToCoreGethGenesis(c *parity.ParityChainSpec) (*genesisT.Genesis, error) {
	...
	if err := confp.Convert(mg, c); err != nil {
		return nil, err
	}
	if err := confp.Convert(mg.Config, c); err != nil {
		return nil, err
	}
	return mg, nil
}
